...
async def get_all_staff(request: Request):
    try:
        ...
        print(e)
        return {"data": [], "status": False, "msg": "Unable to get staff"}
    ...
    allStaff = [
        ...
        for _id, name, cnic, phone_number, email in cursor.fetchall()
    ]

    query2 = (
        "SELECT `username`, `image_path`, role FROM `user` WHERE `role` != 'student'"
    )
...
async def add_staff(
    ...
    try:
        ...
        if cursor.fetchone()[0] > 0 and role == "manager":  # type: ignore
            return {"status": False, "msg": "Manager already exists"}
    except Exception as e:
        print(e)
    ...
    try:
        ...
        print(e)
        return {"status": False, "msg": "Duplicate CNIC or Phone Number"}
...
async def update_staff(request: Request, staff_id: int):
    ...
    try:
        ...
        print(e)
        return {"status": False, "msg": "Unable to update staff"}

    return {"status": True, "msg": "Update staff successful"}
...
async def delete_staff(request: Request, staff_id: int):
    ...
    try:
        ...
        print(e)
        return {"status": False, "msg": "Unable to delete staff"}
    ...
    return {"status": True, "msg": "Staff deleted successfully"}